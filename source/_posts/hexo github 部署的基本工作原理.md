...
啊老问题来了，添加 next 主题在 theme 的时候不可以用 git 来添加，这样会直接，不对，一开始是无法导入，用 submodule 的话那就是……主 repo 会出现空的文件夹导致引用失败 produce 空白界面，去 next release 那边 down 一个最新稳定版本 next 解压到 themes 里面重命名为 next， 这样就好了。
